...
|--------------------------------------------------------------------------
|
| Tất cả các route dành cho Laravel Blade View được đặt trong nhóm 'web'.
| Nhóm này sẽ tự động áp dụng middleware cho session, CSRF, v.v...
|
*/
Route::middleware('web')->group(function () {

    // --- Auth routes (Đăng nhập, Đăng ký, Đăng xuất) ---
    // Cần session và CSRF để hoạt động
    ...
    // Đã đổi prefix từ 'admin-control' thành 'admin'
    Route::prefix('admin-control')->middleware(['auth', IsAdmin::class])->name('admin.')->group(function () {
    ...
    // --- Authenticated user routes (Trang cá nhân, tài khoản...) ---
...
// ROUTE "CATCH-ALL" CHO ỨNG DỤNG REACT
// Route này phải được đặt ở CUỐI CÙNG và bên ngoài nhóm 'web'.
Route::get('/{any}', function () {
    // 'welcome' là file blade chứa <div id="root"></div> của bạn.
    return view('welcome');
    // Regex này đảm bảo route của React không "nuốt" các route đã định nghĩa ở trên.
})->where('any', '^(?!api|admin|login|register).*$');
