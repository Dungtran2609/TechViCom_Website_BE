                            ...
                            <div class="accordion-body">
                                ...
                                @if ($return['status'] === 'pending')
                                    <form action="{{ route('admin.order.process-return', $return['id']) }}" method="POST">
                                        ...
                                        @if ($return['type'] === 'cancel' && $return['order_status'] === 'pending')
                                            <button type="submit" name="action" value="approve" class="btn btn-success btn-sm">Chấp nhận hủy</button>
                                        @elseif ($return['type'] === 'return' && $return['order_status'] === 'delivered')
                                            <button type="submit" name="action" value="approve" class="btn btn-success btn-sm">Chấp nhận trả</button>
                                        @else
                                            <p class="text-danger">
                                                Không thể xử lý: Chỉ đơn hàng Đang chờ xử lý mới được hủy, và chỉ đơn hàng Đã nhận mới được trả.
                                                <br>Trạng thái hiện tại: {{ $return['order_status_vietnamese'] }}
                                            </p>
                                        @endif
