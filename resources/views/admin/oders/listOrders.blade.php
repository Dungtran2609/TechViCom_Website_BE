...
@section('content')
    ...
    <p><strong>Phí vận chuyển:</strong>
                               <span class="shipping-fee-value" data-order-id="{{ $order['id'] }}">
                                   {{ number_format($order['shipping_fee'] ?? 0, 2) }} VND
                               </span>
                            </p>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    accordions.forEach(acc => {
        acc.addEventListener('show.bs.collapse', function () {
            const orderId = this.id.replace('collapse', '');
            const feeElement = this.querySelector('.shipping-fee-value');

            // Kiểm tra đã tính phí hay chưa bằng data attribute hoặc giá trị mặc định
            const currentFeeText = feeElement?.innerText?.replace(/[^\d]/g, '');
            const currentFee = parseInt(currentFeeText, 10);

            if (feeElement?.dataset.loaded === "true" || (currentFee && currentFee > 35000)) return;

            // Đánh dấu đang tải
            feeElement.innerText = 'Đang tính phí...';

            fetch(`/admin/orders/${orderId}/shipping-fee`)
                .then(res => res.json())
                .then(data => {
                    if (data.fee !== undefined) {
                        const formattedFee = new Intl.NumberFormat().format(data.fee) + ' VND';
                        feeElement.innerText = formattedFee;
                        feeElement.dataset.loaded = "true";
                    } else {
                        feeElement.innerText = 'Không có dữ liệu';
                    }
                })
                .catch(err => {
