...
@section('content')
                        ...
                        <div class="mt-4">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="recipient_address" class="form-control"
                                    value="{{ old('recipient_address', $orderData['recipient_address']) }}" @if ($orderData['status'] !== 'pending') readonly @endif>
                            </div>
                            <div class="row mb-3">
                                <div class="col-2">
                                    <strong>Tỉnh:</strong> {{ $orderData['province_name'] ?? 'Chưa có' }}
                                </div>
                                <div class="col-2">
                                    <strong>Quận:</strong> {{ $orderData['district_name'] ?? 'Chưa có' }}
                                </div>
                                <div class="col-2">
                                    <strong>Phường:</strong> {{ $orderData['ward_name'] ?? 'Chưa có' }}
                                </div>
                            </div>

                            <div class="mb-3">
                                <label for="payment_method" class="form-label">Phương thức thanh toán:</label>
                                <select name="payment_method" class="form-control" @if ($orderData['status'] !== 'pending')
                                readonly @endif>
                                    <option value="" disabled {{ !$orderData['payment_method'] ? 'selected' : '' }}>Chọn phương
                                        thức</option>
                                    <option value="credit_card" {{ old('payment_method', $orderData['payment_method']) === 'credit_card' ? 'selected' : '' }}>Thẻ tín dụng/ghi nợ
                                    </option>
                                    <option value="bank_transfer" {{ old('payment_method', $orderData['payment_method']) === 'bank_transfer' ? 'selected' : '' }}>Chuyển khoản ngân
                                        hàng</option>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" name="shipped_at" class="form-control"
                                    value="{{ old('shipped_at', $orderData['shipped_at'] ? $orderData['shipped_at']->format('Y-m-d') : '') }}"
                                    @if ($orderData['status'] !== 'pending') readonly @endif>
                            </div>
                            <div class="mb-3">
                                <label for="shipping_method_id" class="form-label">Phương thức vận chuyển:</label>
                                <select name="shipping_method_id" class="form-control" @if ($orderData['status'] !== 'pending')
                                readonly @endif>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="coupon_id" class="form-control" @if ($orderData['status'] !== 'pending') readonly
                                @endif>
                                    ...
                                    @foreach ($orderData['coupons'] as $coupon)
                                        <option value="{{ $coupon->id }}" {{ old('coupon_id', $orderData['coupon_id']) == $coupon->id ? 'selected' : '' }}>
                                            {{ $coupon->code }}
                                            ({{ $coupon->discount_type === 'percentage' ? $coupon->value . '%' : number_format($coupon->value, 2) . ' VND' }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
        ...
        $itemTotalSum += $itemTotal;
                                            ...
                                            <tr>
                                                ...
                                                <td><input type="number" name="order_items[{{ $index }}][quantity]"
                                                        class="form-control"
                                                        value="{{ old("order_items.$index.quantity", $item['quantity']) }}" min="1"
                                                        @if ($orderData['status'] !== 'pending') readonly @endif></td>
                                                <td><input type="number" step="0.01" name="order_items[{{ $index }}][price]"
                                                        class="form-control"
                                                        value="{{ old("order_items.$index.price", $item['price']) }}" min="0" @if ($orderData['status'] !== 'pending') readonly @endif></td>
                            ...
                            <button type="submit" class="btn btn-primary"
                                onclick="return confirm('Bạn có chắc muốn cập nhật đơn hàng này?')">Cập nhật</button>
                        </div>
                    </form>
    ...
    $itemTotalSum = 0;
        ...
        $itemTotalSum += $itemTotal;
                    ...
                    <p><strong>Tổng tiền sản phẩm (lưu trữ):</strong> {{ number_format($orderData['total_amount'] ?? 0, 2) }}
                        VND</p>
                    @if (abs($itemTotalSum - ($orderData['total_amount'] ?? 0)) > 0.01)
                        <div class="alert alert-warning">
                            ...
                            tiền lưu trữ ({{ number_format($orderData['total_amount'] ?? 0, 2) }} VND).
                        </div>
                    @endif
