        ...
        <form action="{{ route('admin.products.attributes.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="type" id="type" class="form-select @error('type') is-invalid @enderror">
                    <option value="select" @if(old('type', 'select') == 'select') selected @endif>Chọn loại</option>
                    <option value="text" @if(old('type') == 'text') selected @endif>Văn bản</option>
                    <option value="color" @if(old('type') == 'color') selected @endif>Màu sắc</option>
                    <option value="number" @if(old('type') == 'number') selected @endif>Số</option>
                </select>
                @error('type')
                    <div class="invalid-feedback">
                        {{ $message }}
